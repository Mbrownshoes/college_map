...
p1 <- ggplot(cr_simp_alb) +
  ...
  annotation_custom(tableGrob(inst_list, rows=NULL, theme = tabletheme), xmin=1410500, xmax=1970500, ymin=1139000, ymax=1439000) +
...
cty2 <- c('North Vancouver', 'Abbotsford', 'Mission', 'Chilliwack', 'Victoria','Parksville','Ashcroft'
          , 'Agassiz', 'Vancouver','Burnaby')
s1 <- cr_inst1 %>% distinct(City,Institution, .keep_all = TRUE) %>% 
  filter((CR_NUM %in% lm) | (City %in% cty2) )
...
p2 <- ggplot(cr_simp_alb) +
  ...
  geom_text(data = (distinct(cr_inst1,City, .keep_all = TRUE)) %>% 
              filter(!(City %in% c('North Vancouver','Vancouver', 'Burnaby','Agassiz'))),
            aes(label = City, x = lon, y = lat),size=6,color='black',inherit.aes = FALSE,hjust = 1.1, vjust = 1)+
  ...
  geom_text_repel(
    data =  cr_inst1 %>% 
      filter((CR_NUM %in% lm) | (City %in% cty) & (City %in% c('Agassiz', 'Hope'))),
    mapping=aes(label = Short, x = lon, y = lat),
    size=4, box.padding =0.25,
    nudge_x =2
    # nudge_y = ifelse(s1$City == 'Cloverdale', -9, 9)
    # nudge_y=9
  )+
